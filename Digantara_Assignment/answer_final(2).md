...
- **Geocentric Celestial Reference Frame (GCRF)** – the geocentric celestial frame used to represent the spacecraft state and perform orbit dynamics.
- **International Terrestrial Reference Frame (ITRF)** – the Earth-fixed frame used for Earth-relative quantities such as ground tracks, ground-station visibility, and geodetic calculations.
- **Local/orbital RSW (Radial–Along-track–Cross-track)** – useful for interpreting state errors and conjunction geometry.

The orbital elements must be associated with a defined reference frame and epoch; converting them to Cartesian state uses the standard perifocal-to-inertial rotation with a, e, i, Ω, ω, ν.

The transformation between the celestial and terrestrial frames is time-dependent and involves celestial-pole motion (precession/nutation), Earth rotation, and polar motion, using the appropriate Earth Orientation Parameters (EOP) and time scales. The IERS describes this transformation between the Geocentric Celestial Reference System (GCRS) and the International Terrestrial Reference System (ITRS); their practical realizations are the GCRF and ITRF, respectively. [IERS Technical Note 36](https://www.iers.org/SharedDocs/Publikationen/EN/IERS/Publications/tn/TechnNote36/tn36_043.pdf)

The **International Celestial Reference Frame (ICRF)** is the realization of the International Celestial Reference System (ICRS), based on precise VLBI observations of distant extragalactic radio sources; ICRF3 is the current realization. [IERS — ICRF](https://www.iers.org/iers/en/dataproducts/icrf/icrf)

### 1.4 Prediction-accuracy methodology

Meaningful metrics are: position error ||r_pred − r_ref||, velocity error, and the RSW decomposition (radial/along-track/cross-track), plus the full state-error norm and covariance. I assess accuracy at four levels:
